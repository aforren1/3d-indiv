...
class IndividuationStateMachine(Machine):
    def __init__(self):
        states = ['pre_trial', # wait for the space press to start, and show text instruction on the screen
                'moving', # after space press, start countdown timer and show target (5 sec to be within n dist of target)
        ...
        transitions = [pre_trial_t, moving_t, moving_t2, hold_in_target_t, post_trial_t, post_trial_t2]
        Machine.__init__(self, states = states, transitions = transitions, initial = 'pre_trial')
